...
export class AddressCodeComponent implements OnInit {
  ...
  getAddressCodeList(params, filterData) {
  ...
  reloadItems(params) {
    this.getAddressCodeList(params, null);
  }
  ...
  onAddAddressCode() {
    ...
    this.router.navigate(['/addresscode/add']);
  }

  onUpdateAddressCode(item: AddressCode) {
    let navigationExtras: NavigationExtras = {
      skipLocationChange: true,
      
    };
    this.router.navigate(['/addresscode', item.id]);
  }
